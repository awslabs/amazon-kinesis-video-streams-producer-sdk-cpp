...
class TestDeviceInfoProvider : public DefaultDeviceInfoProvider {
  ...
  public:
    TestDeviceInfoProvider(uint64_t device_storage_size, AUTOMATIC_STREAMING_FLAGS automaticStreamingFlags)
        : device_storage_size_(device_storage_size), automaticStreamingFlags_(automaticStreamingFlags)
    {
    }
...
class ProducerTestBase : public ::testing::Test {
  public:
    ProducerTestBase()
        : producer_thread_(0), start_producer_(false), stop_called_(false), frame_dropped_(false), stop_producer_(false), access_key_set_(true),
          buffer_duration_pressure_(false), defaultRegion_(DEFAULT_AWS_REGION), caCertPath_(""), error_status_(STATUS_SUCCESS),
          latency_pressure_count_(0), device_storage_size_(TEST_STORAGE_SIZE_IN_BYTES), fps_(TEST_FPS), total_frame_count_(TEST_TOTAL_FRAME_COUNT),
          current_pressure_state_(OK), buffering_ack_in_sequence_(true), key_frame_interval_(TEST_FPS),
  ...
  protected:
    void handlePressure(std::atomic_bool& pressure_flag, uint32_t grace_period_seconds)
    {
        ...
        if (pressure_flag && current_pressure_state_ == OK) {
            ...
            pressure_cooldown_time_ =
                (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()) + std::chrono::seconds(1))
                    .count();
    ...
    void CreateCredentialProvider()
    {
        ...
        credentials_.reset(new Credentials(std::string(accessKey), std::string(secretKey), sessionTokenStr,
    ...
    void CreateProducer(std::unique_ptr<CredentialProvider> credential_provider, bool cachingEndpointProvider = false,
                        AUTOMATIC_STREAMING_FLAGS automaticStreamingFlags = AUTOMATIC_STREAMING_INTERMITTENT_PRODUCER)
    {
        ...
        try {
            ...
            defaultCallbackProvider.reset(new DefaultCallbackProvider(
                std::move(client_callback_provider_), std::move(stream_callback_provider_), std::move(credential_provider), defaultRegion_,
                EMPTY_STRING, EMPTY_STRING, EMPTY_STRING, caCertPath_,
                cachingEndpointProvider ? API_CALL_CACHE_TYPE_ENDPOINT_ONLY : API_CALL_CACHE_TYPE_NONE, DEFAULT_ENDPOINT_CACHE_UPDATE_PERIOD));

            kinesis_video_producer_ = KinesisVideoProducer::createSync(std::move(device_provider_), std::move(defaultCallbackProvider));
        } catch (std::runtime_error) {
            throw std::runtime_error("Failed");
        }
    }

    void CreateProducer(bool cachingEndpointProvider = false,
                        AUTOMATIC_STREAMING_FLAGS automaticStreamingFlags = AUTOMATIC_STREAMING_INTERMITTENT_PRODUCER)
    {
    ...
    void CreateProducer(API_CALL_CACHE_TYPE api_call_caching, AUTOMATIC_STREAMING_FLAGS automaticStreamingFlags)
    {
        ...
        try {
            std::unique_ptr<DefaultCallbackProvider> defaultCallbackProvider;
            defaultCallbackProvider.reset(new DefaultCallbackProvider(
                std::move(client_callback_provider_), std::move(stream_callback_provider_), std::move(credential_provider_), defaultRegion_,
                EMPTY_STRING, EMPTY_STRING, EMPTY_STRING, caCertPath_, api_call_caching, DEFAULT_ENDPOINT_CACHE_UPDATE_PERIOD));
    ...
    std::shared_ptr<KinesisVideoStream> CreateTestStream(int index, STREAMING_TYPE streaming_type = STREAMING_TYPE_REALTIME,
                                                         ...
                                                         int buffer_duration_seconds = 120)
    {
        ...
        std::unique_ptr<StreamDefinition> stream_definition(new StreamDefinition(
            stream_name, std::chrono::hours(2), &tags, "", streaming_type, "video/h264", std::chrono::milliseconds(max_stream_latency_ms),
            std::chrono::seconds(2), std::chrono::milliseconds(1), true, true, true, true, true, true, true, 0, 25, 4 * 1024 * 1024,
            std::chrono::seconds(buffer_duration_seconds), std::chrono::seconds(buffer_duration_seconds), std::chrono::seconds(50)));

...
static GstStaticPadTemplate srctemplate =
    GST_STATIC_PAD_TEMPLATE("src", GST_PAD_SRC, GST_PAD_ALWAYS, GST_STATIC_CAPS("video/x-h264,stream-format=avc,alignment=au"));
static char const* accessKey;
static char const* secretKey;

static GstElement* setup_kinesisvideoproducersink(void)
{
    ...
    g_object_set(G_OBJECT(kinesisvideoproducersink), "access-key", accessKey, "secret-key", secretKey, NULL);
    return kinesisvideoproducersink;
}

static void cleanup_kinesisvideoproducersink(GstElement* kinesisvideoproducersink)
{
...
GST_START_TEST(check_properties_are_passed_correctly)
{
    ...
    g_object_set(G_OBJECT(pElement), "stream-name", TEST_STREAM_NAME.c_str(), "retention-period", TEST_RETENTION_PERIOD, "streaming-type",
                 TEST_STREAMING_TYPE, "content-type", TEST_CONTENT_TYPE.c_str(), "max-latency", TEST_MAX_LATENCY, "fragment-duration",
                 TEST_FRAGMENT_DURATION, "timecode-scale", TEST_TIMECODE_SCALE, "key-frame-fragmentation", TEST_KEY_FRAME_FRAGMENTATION,
                 "frame-timecodes", TEST_FRAME_TIMECODES, "absolute-fragment-times", TEST_ABSOLUTE_FRAGMENT_TIMES, "fragment-acks",
                 TEST_FRAGMENT_ACKS, "restart-on-error", TEST_RESTART_ON_ERROR, "recalculate-metrics", TEST_RECALCULATE_METRICS, "framerate",
                 TEST_FRAMERATE, "avg-bandwidth-bps", TEST_AVG_BANDWIDTH_BPS, "buffer-duration", TEST_BUFFER_DURATION, "replay-duration",
                 TEST_REPLAY_DURATION, "connection-staleness", TEST_CONNECTION_STALENESS, "codec-id", TEST_CODEC_ID.c_str(), "track-name",
                 TEST_TRACK_NAME.c_str(), "access-key", TEST_ACCESS_KEY.c_str(), "secret-key", TEST_SECRET_KEY.c_str(), "rotation-period",
                 TEST_ROTATION_PERIOD, "log-config", TEST_LOG_CONFIG.c_str(), NULL);

...
typedef struct _CustomData {
    _CustomData()
        : first_video_frame(true), eos_triggered(false), pipeline_blocked(false), stream_status(STATUS_SUCCESS), base_pts(0), max_frame_pts(0),
          total_track_count(1), key_frame_pts(0), current_file_idx(0), last_unpersisted_file_idx(0), kinesis_video_producer(nullptr),
          kinesis_video_stream(nullptr), main_loop(NULL), first_pts(GST_CLOCK_TIME_NONE), use_absolute_fragment_times(true)
    {
...
class SampleStreamCallbackProvider : public StreamCallbackProvider {
  ...
  private:
    ...
    static STATUS streamErrorReportHandler(UINT64 custom_data, STREAM_HANDLE stream_handle, UPLOAD_HANDLE upload_handle, UINT64 errored_timecode,
                                           STATUS status_code);
...
class SampleCredentialProvider : public StaticCredentialProvider {
  ...
  public:
    ...
    void updateCredentials(Credentials& credentials) override
    {
        ...
        auto now_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch());
...
STATUS SampleStreamCallbackProvider::streamConnectionStaleHandler(UINT64 custom_data, STREAM_HANDLE stream_handle, UINT64 last_buffering_ack)
{
...
STATUS
SampleStreamCallbackProvider::streamErrorReportHandler(UINT64 custom_data, STREAM_HANDLE stream_handle, UPLOAD_HANDLE upload_handle,
                                                       UINT64 errored_timecode, STATUS status_code)
{
    LOG_ERROR("Reporting stream error. Errored timecode: " << errored_timecode << " Status: " << status_code);
    ...
    if ((IS_RETRIABLE_ERROR(status_code) && data->uploading_file) || (!IS_RETRIABLE_ERROR(status_code) && !IS_RECOVERABLE_ERROR(status_code))) {
...
STATUS
SampleStreamCallbackProvider::droppedFrameReportHandler(UINT64 custom_data, STREAM_HANDLE stream_handle, UINT64 dropped_frame_timecode)
{
...
STATUS
SampleStreamCallbackProvider::fragmentAckReceivedHandler(UINT64 custom_data, STREAM_HANDLE stream_handle, UPLOAD_HANDLE upload_handle,
...
void create_kinesis_video_frame(Frame* frame, const nanoseconds& pts, const nanoseconds& dts, FRAME_FLAGS flags, void* data, size_t len,
                                UINT64 track_id)
{
    frame->flags = flags;
...
static GstFlowReturn on_new_sample(GstElement* sink, CustomData* data)
{
    ...
    if (!data->stream_started[track_id]) {
        ...
        if (!all_stream_started(data)) {
            ...
            data->audio_video_sync_cv.wait_for(lk, seconds(DEFAULT_AUDIO_VIDEO_DRIFT_TIMEOUT_SECOND), [data] { return all_stream_started(data); });
    ...
    dropFrame = GST_BUFFER_FLAG_IS_SET(buffer, GST_BUFFER_FLAG_CORRUPTED) || GST_BUFFER_FLAG_IS_SET(buffer, GST_BUFFER_FLAG_DECODE_ONLY) ||
    ...
    create_kinesis_video_frame(&frame, std::chrono::nanoseconds(buffer->pts), std::chrono::nanoseconds(buffer->dts), kinesis_video_flags, info.data,
                               info.size, track_id);
...
void kinesis_video_init(CustomData* data)
{
    ...
    if (nullptr != (accessKey = getenv(ACCESS_KEY_ENV_VAR)) && nullptr != (secretKey = getenv(SECRET_KEY_ENV_VAR))) {
        ...
        data->credential.reset(
            new Credentials(string(accessKey), string(secretKey), sessionTokenStr, std::chrono::seconds(DEFAULT_CREDENTIAL_EXPIRATION_SECONDS)));
        credential_provider.reset(new SampleCredentialProvider(*data->credential.get()));

    } else if (nullptr != (iot_get_credential_endpoint = getenv("IOT_GET_CREDENTIAL_ENDPOINT")) && nullptr != (cert_path = getenv("CERT_PATH")) &&
        ...
        credential_provider.reset(
            new IotCertCredentialProvider(iot_get_credential_endpoint, cert_path, private_key_path, role_alias, ca_cert_path, data->stream_name));

    } else {
        LOG_AND_THROW("No valid credential method was found");
    }

    data->kinesis_video_producer =
        KinesisVideoProducer::createSync(std::move(device_info_provider), std::move(client_callback_provider), std::move(stream_callback_provider),
                                         std::move(credential_provider), API_CALL_CACHE_TYPE_ALL, defaultRegionStr);
...
void kinesis_video_stream_init(CustomData* data)
{
    ...
    unique_ptr<StreamDefinition> stream_definition(new StreamDefinition(
        data->stream_name, hours(DEFAULT_RETENTION_PERIOD_HOURS), nullptr, DEFAULT_KMS_KEY_ID, streaming_type, DEFAULT_CONTENT_TYPE,
        duration_cast<milliseconds>(seconds(DEFAULT_MAX_LATENCY_SECONDS)), milliseconds(DEFAULT_FRAGMENT_DURATION_MILLISECONDS),
        milliseconds(DEFAULT_TIMECODE_SCALE_MILLISECONDS), DEFAULT_KEY_FRAME_FRAGMENTATION, DEFAULT_FRAME_TIMECODES,
        data->use_absolute_fragment_times, DEFAULT_FRAGMENT_ACKS, DEFAULT_RESTART_ON_ERROR, DEFAULT_RECALCULATE_METRICS, true,
        NAL_ADAPTATION_FLAG_NONE, DEFAULT_STREAM_FRAMERATE, DEFAULT_AVG_BANDWIDTH_BPS, seconds(DEFAULT_BUFFER_DURATION_SECONDS),
        seconds(DEFAULT_REPLAY_DURATION_SECONDS), seconds(DEFAULT_CONNECTION_STALENESS_SECONDS), DEFAULT_CODEC_ID, DEFAULT_TRACKNAME, nullptr, 0,
        MKV_TRACK_INFO_TYPE_VIDEO, vector<uint8_t>(), DEFAULT_VIDEO_TRACKID));
...
int gstreamer_init(int argc, char* argv[], CustomData& data)
{
    ...
    if (data.uploading_file) {
        ...
        gst_bin_add_many(GST_BIN(pipeline), appsink_video, appsink_audio, filesrc, demux, h264parse, aac_parse, video_queue, audio_queue,
                         video_filter, audio_filter, NULL);
        ...
        if (!gst_element_link_many(video_queue, h264parse, video_filter, appsink_video, NULL)) {
            g_printerr("Video elements could not be linked.\n");
            gst_object_unref(pipeline);
            return 1;
        }

        if (!gst_element_link_many(audio_queue, aac_parse, audio_filter, appsink_audio, NULL)) {
        ...
        if (!audio_device.empty()) {
            ...
            LOG_ERROR("No audio device found. Please do export " << AUDIO_DEVICE_ENV_VAR
                                                                 << "=audio_device to config audio device (e.g. export AWS_KVS_AUDIO_DEVICE=hw:1,0)");
            return 1;
        }
        ...
        if (!videosrc || !h264enc || !audiosrc || !audioconvert || !aac_enc || !videoconvert || !audio_resample || !audio_src_filter ||
            !video_src_filter || !h264parse || !aac_parse) {
            g_printerr("Not all elements could be created:\n");
            return 1;
        }

        gst_bin_add_many(GST_BIN(pipeline), appsink_video, appsink_audio, videosrc, h264enc, audiosrc, audioconvert, aac_enc, videoconvert,
                         video_queue, audio_queue, video_filter, audio_filter, audio_resample, audio_src_filter, video_src_filter, h264parse,
                         aac_parse, NULL);

        if (!gst_element_link_many(videosrc, videoconvert, video_src_filter, h264enc, h264parse, video_filter, video_queue, appsink_video, NULL)) {
            g_printerr("Video elements could not be linked.\n");
            gst_object_unref(pipeline);
            return 1;
        }
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2) {
        LOG_ERROR("Usage: AWS_ACCESS_KEY_ID=SAMPLEKEY AWS_SECRET_ACCESS_KEY=SAMPLESECRET ./kinesis_video_gstreamer_audio_video_sample_app "
                  "my-stream-name /path/to/file"

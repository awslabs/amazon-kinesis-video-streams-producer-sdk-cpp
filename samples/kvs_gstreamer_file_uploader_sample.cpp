...
static const char* AVAILABLE_PROPERTIES[] = {PROPERTY_PREFIX "IOT_CERTIFICATE",
                                             "Use aws iot certificate to obtain credentials",
                                             PROPERTY_PREFIX "RETENTION_PERIOD",
                                             "Length of time stream is preserved. Unit: hours",
                                             PROPERTY_PREFIX "STORAGE_SIZE",
...
int gstreamer_init(int argc, char* argv[], CustomData* data)
{
    ...
    if (data->content_type == CONTENT_TYPE_VIDEO_ONLY) { // video only
        ret = snprintf(pipeline_buf, sizeof(pipeline_buf), "filesrc location=%s ! %s ! h264parse ! video/x-h264,stream-format=avc,alignment=au ! %s",
...
string build_kvssink_str(string stream_name, unsigned long long file_start_time)
{
    ...
    ss << KVS_SINK_PLUGIN_NAME << " stream-name=" << stream_name << " streaming-type=" DEFAULT_STREAMING_TYPE
...
void print_usage(char* program_path)
{
    ...
    printf("USAGE\n"
           ...
           "        "
           "IOT_CERTIFICATE=\"iot-certificate,endpoint=endpoint,cert-path=/path/to/certificate,key-path=/path/to/private/key,ca-path=/path/to/"
           "ca-cert,role-aliases=role-aliases\" \\\n"
...
int main(int argc, char* argv[])
{
    ...
    do {
        ...
        if (ret != 0) {
            LOG_ERROR("Failed to initialize gstreamer pipeline. Have you set GST_PLUGIN_PATH properly?\n\n"

...
static GstFlowReturn on_new_sample(GstElement* sink, CustomData* data)
{
    ...
    if (!isDroppable) {
        ...
        if (false ==
            put_frame(data->kinesis_video_stream_handles[stream_handle_key], data->frame_data_map[stream_handle_key], buffer_size,
                      std::chrono::nanoseconds(buffer->pts), std::chrono::nanoseconds(buffer->dts), kinesis_video_flags)) {
...
void kinesis_video_init(CustomData* data)
{
    ...
    credentials_.reset(new Credentials(string(accessKey), string(secretKey), sessionTokenStr, std::chrono::seconds(180)));
...
void kinesis_stream_init(string stream_name, CustomData* data, string stream_handle_key)
{
    ...
    unique_ptr<StreamDefinition> stream_definition(new StreamDefinition(
        stream_name.c_str(), hours(DEFAULT_RETENTION_PERIOD_HOURS), nullptr, DEFAULT_KMS_KEY_ID, DEFAULT_STREAMING_TYPE, DEFAULT_CONTENT_TYPE,
        duration_cast<milliseconds>(seconds(DEFAULT_MAX_LATENCY_SECONDS)), milliseconds(DEFAULT_FRAGMENT_DURATION_MILLISECONDS),
        milliseconds(DEFAULT_TIMECODE_SCALE_MILLISECONDS), DEFAULT_KEY_FRAME_FRAGMENTATION, DEFAULT_FRAME_TIMECODES, DEFAULT_ABSOLUTE_FRAGMENT_TIMES,
        DEFAULT_FRAGMENT_ACKS, DEFAULT_RESTART_ON_ERROR, DEFAULT_RECALCULATE_METRICS, true, NAL_ADAPTATION_FLAG_NONE, DEFAULT_STREAM_FRAMERATE,
        DEFAULT_AVG_BANDWIDTH_BPS, seconds(DEFAULT_BUFFER_DURATION_SECONDS), seconds(DEFAULT_REPLAY_DURATION_SECONDS),
...
int gstreamer_init(int argc, char* argv[])
{
    ...
    if (argc < 3) {
        LOG_ERROR("Usage: AWS_ACCESS_KEY_ID=SAMPLEKEY AWS_SECRET_ACCESS_KEY=SAMPLESECRET ./kinesis_video_gstreamer_sample_multistream_app "
                  "base-stream-name rtsp-url-file-name\n"
                  << "base-stream-name: the application will create one stream for each rtsp url read. The base-stream-names will be suffixed with "
                     "indexes to differentiate the streams\n"
    ...
    for (int i = 0; i < rtsp_urls.size(); ++i) {
        ...
        g_object_set(G_OBJECT(source), "location", rtsp_urls[i].c_str(), "short-header", true, // Necessary for target camera
                     NULL);

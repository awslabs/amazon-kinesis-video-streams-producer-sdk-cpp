...
static const std::set<std::string> iot_param_set = {
    IOT_GET_CREDENTIAL_ENDPOINT, CERTIFICATE_PATH,      PRIVATE_KEY_PATH, CA_CERT_PATH, ROLE_ALIASES, IOT_THING_NAME,
    IOT_CONNECTION_TIMEOUT,      IOT_COMPLETION_TIMEOUT};

static const time_t time_point = std::time(NULL);
static const long timezone_offset = static_cast<long>(std::mktime(std::gmtime(&time_point)) - std::mktime(std::localtime(&time_point)));
...
gboolean parseIotCredentialGstructure(GstStructure* g_struct, std::map<std::string, std::string>& iot_cert_params)
{
    ...
    for (std::map<std::string, std::string>::iterator it = iot_cert_params.begin(); it != iot_cert_params.end(); ++it) {
    ...
    if (params_key_set != iot_param_set) {
        ...
        LOG_ERROR("Missing parameters for iot certificate credential. The following keys are expected" << ostream.str());
        ret = FALSE;
    }

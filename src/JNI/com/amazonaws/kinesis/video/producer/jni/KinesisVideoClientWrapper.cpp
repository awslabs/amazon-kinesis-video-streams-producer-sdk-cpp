...
KinesisVideoClientWrapper::KinesisVideoClientWrapper(JNIEnv* env, jobject thiz, jobject deviceInfo) : mClientHandle(INVALID_CLIENT_HANDLE_VALUE)
{
    UINT32 retStatus;
...
void KinesisVideoClientWrapper::getKinesisVideoMetrics(jobject kinesisVideoMetrics)
{
    ...
    env->CallVoidMethod(kinesisVideoMetrics, setterMethodId, metrics.contentStoreSize, metrics.contentStoreAllocatedSize,
                        metrics.contentStoreAvailableSize, metrics.totalContentViewsSize, metrics.totalFrameRate, metrics.totalTransferRate);
}
...
void KinesisVideoClientWrapper::getKinesisVideoStreamMetrics(jlong streamHandle, jobject kinesisVideoStreamMetrics)
{
    ...
    // call the setter method
    env->CallVoidMethod(kinesisVideoStreamMetrics, setterMethodId, metrics.overallViewSize, metrics.currentViewSize, metrics.overallViewDuration,
                        metrics.currentViewDuration, metrics.currentFrameRate, metrics.currentTransferRate);
}
...
void KinesisVideoClientWrapper::getKinesisVideoStreamData(jlong streamHandle, jlong uploadHandle, jobject dataBuffer, jint offset, jint length,
                                                          jobject readResult)
{
    STATUS retStatus = STATUS_SUCCESS;
    JNIEnv* env;
    ...
    if (STATUS_SUCCESS != retStatus && STATUS_AWAITING_PERSISTED_ACK != retStatus && STATUS_UPLOAD_HANDLE_ABORTED != retStatus &&
        STATUS_NO_MORE_DATA_AVAILABLE != retStatus && STATUS_END_OF_STREAM != retStatus) {
        char errMessage[256];
        SNPRINTF(errMessage, 256, "Failed to get data from the stream 0x%016" PRIx64 " with uploadHandle %" PRIu64, (UINT64) streamHandle,
                 (UINT64) uploadHandle);
    ...
    // Call the setter method
    env->CallVoidMethod(readResult, setterMethodId, filledSize, isEos);
...
void KinesisVideoClientWrapper::describeStreamResult(jlong streamHandle, jint httpStatusCode, jobject streamDescription)
{
    ...
    if (NULL != streamDescription) {
        if (!setStreamDescription(env, streamDescription, &streamDesc)) {
            ...
            throwNativeException(env, EXCEPTION_NAME, "Failed converting stream description object.", STATUS_INVALID_OPERATION);
            return;
        }
...
void KinesisVideoClientWrapper::getStreamingTokenResult(jlong streamHandle, jint httpStatusCode, jbyteArray streamingToken, jint tokenSize,
                                                        jlong expiration)
{
    STATUS retStatus = STATUS_SUCCESS;
    JNIEnv* env;
    ...
    if (STATUS_FAILED(retStatus = ::getStreamingTokenResultEvent(streamHandle, (SERVICE_CALL_RESULT) httpStatusCode, pToken, (UINT32) tokenSize,
                                                                 (UINT64) expiration))) {
...
void KinesisVideoClientWrapper::deviceCertToTokenResult(jlong clientHandle, jint httpStatusCode, jbyteArray token, jint tokenSize, jlong expiration)
{
    ...
    if (STATUS_FAILED(retStatus = ::deviceCertToTokenResultEvent(clientHandle, (SERVICE_CALL_RESULT) httpStatusCode, pToken, (UINT32) tokenSize,
                                                                 (UINT64) expiration))) {
...
BOOL KinesisVideoClientWrapper::setCallbacks(JNIEnv* env, jobject thiz)
{
    ...
    mFragmentAckReceivedMethodId =
        env->GetMethodID(thizCls, "fragmentAckReceived", "(JJLcom/amazonaws/kinesisvideo/producer/KinesisVideoFragmentAck;)V");
    ...
    mCreateStreamMethodId =
        env->GetMethodID(thizCls, "createStream", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;JJJ[BIJ)I");
...
STATUS KinesisVideoClientWrapper::fragmentAckReceivedFunc(UINT64 customData, STREAM_HANDLE streamHandle, UPLOAD_HANDLE upload_handle,
                                                          PFragmentAck pFragmentAck)
{
    ...
    ack = env->NewObject(ackClass, methodId, (jint) pFragmentAck->ackType, (jlong) pFragmentAck->timestamp, jstrSequenceNum,
...
STATUS KinesisVideoClientWrapper::streamErrorReportFunc(UINT64 customData, STREAM_HANDLE streamHandle, UPLOAD_HANDLE upload_handle,
                                                        UINT64 fragmentTimecode, STATUS statusCode)
{
    ...
    // Call the Java func
    env->CallVoidMethod(pWrapper->mGlobalJniObjRef, pWrapper->mStreamErrorReportMethodId, streamHandle, upload_handle, fragmentTimecode, statusCode);
    CHK_JVM_EXCEPTION(env);
...
STATUS KinesisVideoClientWrapper::streamDataAvailableFunc(UINT64 customData, STREAM_HANDLE streamHandle, PCHAR streamName, UINT64 uploadHandle,
                                                          UINT64 duration, UINT64 availableSize)
{
    ...
    env->CallVoidMethod(pWrapper->mGlobalJniObjRef, pWrapper->mStreamDataAvailableMethodId, streamHandle, NULL, uploadHandle, duration,
                        availableSize);
    CHK_JVM_EXCEPTION(env);
...
STATUS KinesisVideoClientWrapper::createStreamFunc(UINT64 customData, PCHAR deviceName, PCHAR streamName, PCHAR contentType, PCHAR kmsKeyId,
                                                   UINT64 retention, PServiceCallContext pCallbackContext)
{
    ...
    if (jstrContentType == NULL || jstrDeviceName == NULL || jstrStreamName == NULL || authByteArray == NULL) {
        retStatus = STATUS_NOT_ENOUGH_MEMORY;
        goto CleanUp;
    }

    // Copy the bits into the managed array
    env->SetByteArrayRegion(authByteArray, 0, pCallbackContext->pAuthInfo->size, (const jbyte*) pCallbackContext->pAuthInfo->data);

    // Invoke the callback
    retStatus = env->CallIntMethod(pWrapper->mGlobalJniObjRef, pWrapper->mCreateStreamMethodId, jstrDeviceName, jstrStreamName, jstrContentType,
                                   jstrKmsKeyId, retention, pCallbackContext->callAfter, pCallbackContext->timeout, authByteArray,
                                   pCallbackContext->pAuthInfo->type, pCallbackContext->customData);
...
STATUS KinesisVideoClientWrapper::describeStreamFunc(UINT64 customData, PCHAR streamName, PServiceCallContext pCallbackContext)
{
    ...
    jstrStreamName = env->NewStringUTF(streamName);
    authByteArray = env->NewByteArray(pCallbackContext->pAuthInfo->size);
    if (jstrStreamName == NULL || authByteArray == NULL) {
        retStatus = STATUS_NOT_ENOUGH_MEMORY;
        goto CleanUp;
    }
    ...
    // Invoke the callback
    retStatus = env->CallIntMethod(pWrapper->mGlobalJniObjRef, pWrapper->mDescribeStreamMethodId, jstrStreamName, pCallbackContext->callAfter,
                                   pCallbackContext->timeout, authByteArray, pCallbackContext->pAuthInfo->type, pCallbackContext->customData);
...
STATUS KinesisVideoClientWrapper::getStreamingEndpointFunc(UINT64 customData, PCHAR streamName, PCHAR apiName, PServiceCallContext pCallbackContext)
{
    ...
    authByteArray = env->NewByteArray(pCallbackContext->pAuthInfo->size);
    if (jstrStreamName == NULL || jstrApiName == NULL || authByteArray == NULL) {
        retStatus = STATUS_NOT_ENOUGH_MEMORY;
        goto CleanUp;
    }
    ...
    // Invoke the callback
    retStatus = env->CallIntMethod(pWrapper->mGlobalJniObjRef, pWrapper->mGetStreamingEndpointMethodId, jstrStreamName, jstrApiName,
                                   pCallbackContext->callAfter, pCallbackContext->timeout, authByteArray, pCallbackContext->pAuthInfo->type,
...
STATUS KinesisVideoClientWrapper::getStreamingTokenFunc(UINT64 customData, PCHAR streamName, STREAM_ACCESS_MODE accessMode,
                                                        PServiceCallContext pCallbackContext)
{
    ...
    // Invoke the callback
    retStatus = env->CallIntMethod(pWrapper->mGlobalJniObjRef, pWrapper->mGetStreamingTokenMethodId, jstrStreamName, pCallbackContext->callAfter,
                                   pCallbackContext->timeout, authByteArray, pCallbackContext->pAuthInfo->type, pCallbackContext->customData);
...
STATUS KinesisVideoClientWrapper::putStreamFunc(UINT64 customData, PCHAR streamName, PCHAR containerType, UINT64 streamStartTime,
                                                ...
                                                PServiceCallContext pCallbackContext)
{
    ...
    authByteArray = env->NewByteArray(pCallbackContext->pAuthInfo->size);
    if (jstrStreamName == NULL || jstrContainerType == NULL || jstrStreamingEndpoint == NULL || authByteArray == NULL) {
        retStatus = STATUS_NOT_ENOUGH_MEMORY;
        goto CleanUp;
    }
    ...
    // Invoke the callback
    retStatus = env->CallIntMethod(pWrapper->mGlobalJniObjRef, pWrapper->mPutStreamMethodId, jstrStreamName, jstrContainerType, streamStartTime,
                                   absoluteFragmentTimestamp == TRUE, ackRequired == TRUE, jstrStreamingEndpoint, pCallbackContext->callAfter,
                                   pCallbackContext->timeout, authByteArray, pCallbackContext->pAuthInfo->type, pCallbackContext->customData);
...
STATUS KinesisVideoClientWrapper::tagResourceFunc(UINT64 customData, PCHAR streamArn, UINT32 tagCount, PTag tags,
                                                  PServiceCallContext pCallbackContext)
{
    JNIEnv* env;
    BOOL detached = FALSE;
    ...
    // Invoke the callback
    retStatus = env->CallIntMethod(pWrapper->mGlobalJniObjRef, pWrapper->mTagResourceMethodId, jstrStreamArn, tagArray, pCallbackContext->callAfter,
                                   pCallbackContext->timeout, authByteArray, pCallbackContext->pAuthInfo->type, pCallbackContext->customData);
...
STATUS KinesisVideoClientWrapper::createDeviceFunc(UINT64 customData, PCHAR deviceName, PServiceCallContext pCallbackContext)
{
    ...
    // Invoke the callback
    retStatus = env->CallIntMethod(pWrapper->mGlobalJniObjRef, pWrapper->mCreateDeviceMethodId, jstrDeviceName, pCallbackContext->callAfter,
                                   pCallbackContext->timeout, authByteArray, pCallbackContext->pAuthInfo->type, pCallbackContext->customData);
...
STATUS KinesisVideoClientWrapper::deviceCertToTokenFunc(UINT64 customData, PCHAR deviceName, PServiceCallContext pCallbackContext)
{
    ...
    // Invoke the callback
    retStatus = env->CallIntMethod(pWrapper->mGlobalJniObjRef, pWrapper->mDeviceCertToTokenMethodId, jstrDeviceName, pCallbackContext->callAfter,
                                   pCallbackContext->timeout, authByteArray, pCallbackContext->pAuthInfo->type, pCallbackContext->customData);
...
AUTH_INFO_TYPE KinesisVideoClientWrapper::authInfoTypeFromInt(UINT32 authInfoType)
{
    switch (authInfoType) {
        case 1:
            return AUTH_INFO_TYPE_CERT;
        case 2:
            return AUTH_INFO_TYPE_STS;
        case 3:
            return AUTH_INFO_NONE;
        default:
            return AUTH_INFO_UNDEFINED;
    }
}
...
VOID KinesisVideoClientWrapper::logPrintFunc(UINT32 level, PCHAR tag, PCHAR fmt, ...)
{
    ...
    2021-12-10 10:01:53,874 [main] TRACE c.a.k.j.c.KinesisVideoJavaClientFactory - [PIC] KinesisVideoProducerJNI -
    Java_com_amazonaws_kinesisvideo_internal_producer_jni_NativeKinesisVideoProducerJni_createKinesisVideoStream(): Enter 2021-12-10 10:01:53,875
    [main] INFO  c.a.k.j.c.KinesisVideoJavaClientFactory - [PIC] KinesisVideoProducerJNI -
    ...
    2021-12-10 10:01:53,875 [main] INFO  c.a.k.j.c.KinesisVideoJavaClientFactory - [PIC] KinesisVideoClient - createKinesisVideoStream(): Creating
    Kinesis Video Stream. 2021-12-10 10:01:53,875 [main] DEBUG c.a.k.j.c.KinesisVideoJavaClientFactory - [PIC] Stream - logStreamInfo(): Kinesis Video
    Stream Info
    ...
    2021-12-10 10:01:53,875 [main] DEBUG c.a.k.j.c.KinesisVideoJavaClientFactory - [PIC] Stream - logStreamInfo(): 	Streaming type:
    STREAMING_TYPE_REALTIME 2021-12-10 10:01:53,876 [main] DEBUG c.a.k.j.c.KinesisVideoJavaClientFactory - [PIC] Stream - logStreamInfo(): 	Content
    type: video/h264
    */

...
StreamDefinition::StreamDefinition(string stream_name, duration<uint64_t, ratio<3600>> retention_period, const map<string, string>* tags,
                                   string kms_key_id, STREAMING_TYPE streaming_type, string content_type, duration<uint64_t, milli> max_latency,
                                   duration<uint64_t, milli> fragment_duration, duration<uint64_t, milli> timecode_scale,
                                   bool key_frame_fragmentation, bool frame_timecodes, bool absolute_fragment_times, bool fragment_acks,
                                   bool restart_on_error, bool recalculate_metrics, bool allowStreamCreation, uint32_t nal_adaptation_flags,
                                   uint32_t frame_rate, uint32_t avg_bandwidth_bps, duration<uint64_t> buffer_duration,
                                   duration<uint64_t> replay_duration, duration<uint64_t> connection_staleness, string codec_id, string track_name,
                                   const uint8_t* codecPrivateData, uint32_t codecPrivateDataSize, MKV_TRACK_INFO_TYPE track_type,
                                   const vector<uint8_t> segment_uuid, const uint64_t default_track_id,
                                   CONTENT_STORE_PRESSURE_POLICY contentStorePressurePolicy, CONTENT_VIEW_OVERFLOW_POLICY contentViewOverflowPolicy)
    : tags_(tags), stream_name_(stream_name)
{
    ...
    stream_info_.retention = duration_cast<nanoseconds>(retention_period).count() / DEFAULT_TIME_UNIT_IN_NANOS;
    ...
    stream_info_.streamCaps.maxLatency = duration_cast<nanoseconds>(max_latency).count() / DEFAULT_TIME_UNIT_IN_NANOS;
    stream_info_.streamCaps.fragmentDuration = duration_cast<nanoseconds>(fragment_duration).count() / DEFAULT_TIME_UNIT_IN_NANOS;
    stream_info_.streamCaps.timecodeScale = duration_cast<nanoseconds>(timecode_scale).count() / DEFAULT_TIME_UNIT_IN_NANOS;
    ...
    stream_info_.streamCaps.bufferDuration = duration_cast<nanoseconds>(buffer_duration).count() / DEFAULT_TIME_UNIT_IN_NANOS;
    stream_info_.streamCaps.replayDuration = duration_cast<nanoseconds>(replay_duration).count() / DEFAULT_TIME_UNIT_IN_NANOS;
    stream_info_.streamCaps.connectionStalenessDuration = duration_cast<nanoseconds>(connection_staleness).count() / DEFAULT_TIME_UNIT_IN_NANOS;
...
void StreamDefinition::addTrack(const uint64_t track_id, const string& track_name, const string& codec_id, MKV_TRACK_INFO_TYPE track_type,

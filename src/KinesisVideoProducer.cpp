...
unique_ptr<KinesisVideoProducer> KinesisVideoProducer::create(unique_ptr<DeviceInfoProvider> device_info_provider,
                                                              unique_ptr<ClientCallbackProvider> client_callback_provider,
                                                              unique_ptr<StreamCallbackProvider> stream_callback_provider,
                                                              unique_ptr<CredentialProvider> credential_provider, const std::string& region,
                                                              const std::string& control_plane_uri, const std::string& user_agent_name)
{
    unique_ptr<DefaultCallbackProvider> callback_provider(new DefaultCallbackProvider(
        std::move(client_callback_provider), std::move(stream_callback_provider), std::move(credential_provider), region, control_plane_uri,
        user_agent_name, device_info_provider->getCustomUserAgent(), EMPTY_STRING, false, DEFAULT_ENDPOINT_CACHE_UPDATE_PERIOD));
...
unique_ptr<KinesisVideoProducer> KinesisVideoProducer::createSync(unique_ptr<DeviceInfoProvider> device_info_provider,
                                                                  unique_ptr<ClientCallbackProvider> client_callback_provider,
                                                                  unique_ptr<StreamCallbackProvider> stream_callback_provider,
                                                                  unique_ptr<CredentialProvider> credential_provider, const std::string& region,
                                                                  const std::string& control_plane_uri, const std::string& user_agent_name,
                                                                  bool is_caching_endpoint, uint64_t caching_update_period)
{
    unique_ptr<DefaultCallbackProvider> callback_provider(
        new DefaultCallbackProvider(std::move(client_callback_provider), std::move(stream_callback_provider), std::move(credential_provider), region,
                                    control_plane_uri, user_agent_name, device_info_provider->getCustomUserAgent(),
                                    device_info_provider->getCertPath(), is_caching_endpoint, caching_update_period));
...
unique_ptr<KinesisVideoProducer> KinesisVideoProducer::createSync(unique_ptr<DeviceInfoProvider> device_info_provider,
                                                                  ...
                                                                  API_CALL_CACHE_TYPE api_call_caching, const std::string& region,
                                                                  const std::string& control_plane_uri, const std::string& user_agent_name,
                                                                  uint64_t caching_update_period)
{
    unique_ptr<DefaultCallbackProvider> callback_provider(new DefaultCallbackProvider(
        std::move(client_callback_provider), std::move(stream_callback_provider), std::move(credential_provider), region, control_plane_uri,
        user_agent_name, device_info_provider->getCustomUserAgent(), device_info_provider->getCertPath(), api_call_caching, caching_update_period));
...
shared_ptr<KinesisVideoStream> KinesisVideoProducer::createStream(unique_ptr<StreamDefinition> stream_definition)
{
    ...
    std::shared_ptr<KinesisVideoStream> kinesis_video_stream(new KinesisVideoStream(*this, stream_definition->getStreamName()),
                                                             KinesisVideoStream::videoStreamDeleter);
    STATUS status = createKinesisVideoStream(client_handle_, &stream_info, kinesis_video_stream->getStreamHandle());

    if (STATUS_FAILED(status)) {
        stringstream status_strstrm;
        status_strstrm << std::hex << status;
        LOG_AND_THROW(" Unable to create Kinesis Video stream. " + stream_definition->getStreamName() + " Error status: 0x" + status_strstrm.str());
    }

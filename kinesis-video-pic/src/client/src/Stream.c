...
STATUS freeStream(PKinesisVideoStream pKinesisVideoStream)
{
    ...
    // Release the Ready state condition variable if set
    ...
    // Release the stream closed notifier condition variable if set
...
STATUS streamStartFixupOnReconnect(PKinesisVideoStream pKinesisVideoStream)
{
    ...
    UINT64 allocSize, currentDuration, windowDuration;
    ...
    CHK_STATUS(contentViewGetWindowDuration(pKinesisVideoStream->pView, &currentDuration, &windowDuration));
    CHK(currentDuration != 0, retStatus);
...
STATUS resetStream(PKinesisVideoStream pKinesisVideoStream)
{
    ...
    if (pKinesisVideoStream->streamReady) {
        ...
        pKinesisVideoStream->streamState = STREAM_STATE_NEW;
        CHK_STATUS(getStateMachineState(pKinesisVideoStream->base.pStateMachine, STREAM_STATE_NEW, &pState));
        pKinesisVideoStream->base.pStateMachine->context.pCurrentState = pState;
    }
    ...
    // Set grace period to TRUE to avoid putFrame thread triggering mkv header regeneration due to token expiration
    // and cause the new putMedia connection created by resetStream to get end-of-stream prematurely.
    pKinesisVideoStream->gracePeriod = TRUE;
    ...
    if (pKinesisVideoStream->base.pStateMachine != NULL) {
        pKinesisVideoStream->base.pStateMachine->context.retryCount = 0;
        pKinesisVideoStream->base.pStateMachine->context.time = 0;
    }

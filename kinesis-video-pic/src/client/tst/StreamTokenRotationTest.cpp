...
TEST_F(StreamTokenRotationTest, stopStateFromCreate)
{
    UINT32 i, filledSize, rotation, lastRotation;
    BYTE tempBuffer[1000];
    BYTE getDataBuffer[5000];
    UINT64 timestamp, clientStreamHandle, oldClientStreamHandle;
    Frame frame;
    STATUS status;
    ...
    for (i = 0, timestamp = 0; timestamp <= runDuration; timestamp += TEST_LONG_FRAME_DURATION, i++) {
        ...
        // Check for rotation
        if (timestamp > rotation * MIN_STREAMING_TOKEN_EXPIRATION_DURATION - STREAMING_TOKEN_EXPIRATION_GRACE_PERIOD + TEST_LONG_FRAME_DURATION) {

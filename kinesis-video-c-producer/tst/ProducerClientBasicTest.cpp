...
{
    UINT32 index = (UINT32) (UINT64) arg;
...
{
    UINT32 streamIndex = (UINT32) (UINT64) arg;
    ProducerClientBasicTest* pTest = (ProducerClientBasicTest*) gProducerClientTestBase;
    ...
    if (streamIndex == TEST_STREAM_COUNT - 1) {
...
TEST_F(ProducerClientBasicTest, create_produce_stream_parallel)
{
    ...
    for (UINT32 i = 0; i < TEST_STREAM_COUNT; i++) {
        // Create the streams
        THREAD_CREATE(&mProducerThread, staticCreateProducerRoutine, (PVOID) (UINT64) i);
...
TEST_F(ProducerClientBasicTest, create_produce_client_parallel)
{
    ...
    for (UINT32 i = 0; i < TEST_STREAM_COUNT; i++) {
        // Create the streams
        THREAD_CREATE(&mProducerThread, staticCreateProducerClientRoutine, (PVOID) (UINT64) i);
        THREAD_SLEEP(300 * HUNDREDS_OF_NANOS_IN_A_MILLISECOND);
    }
    ...
    for (UINT32 i = 0; i < TEST_STREAM_COUNT; i++) {
        EXPECT_EQ(STATUS_SUCCESS, THREAD_CREATE(&mProducerThread, staticProducerClientStartRoutine, (PVOID) (UINT64) i));
    }
